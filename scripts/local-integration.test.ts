...
describe("local Docker integration smoke", () => {
  ...
  test("uses a local JWKS fixture and signed HOP-1 JWT", async () => {
    ...
    expect(smoke).toContain(
      "AGENTGATEWAY_IMAGE=${LOCAL_AGENTGATEWAY_IMAGE:-ghcr.io/apelogic-ai/mcp-gw-agentgateway:0.4.10}",
    );
    ...
    expect(smoke).toContain('bun "$ROOT_DIR/scripts/fixtures/oauth-scope-recovery-postgres.ts"');
