...
export class ConnectionLifecycle {
  ...
  async recoverFromProviderAuthenticationFailure(
    ...
    this.metric({ name: "provider_auth_rejection", provider: this.providerId, value: 1 });
    const observed = await this.options.store.getConnection(
      this.providerId,
      identity.issuer,
      identity.subject,
    ...
    if (rejectedActiveCredential) {
      ...
      if (
        observedActive &&
        observedActive !== rejectedActiveCredential &&
        scopesSatisfied(this.options.adapter, observed.grantedScopes, requiredScopes)
      ) {
  ...
  private async renew(
    ...
    this.metric({ name: "renewal_attempt", provider: this.providerId, operation, value: 1 });
    let issuedPreparation: Extract<RenewalPreparation, { kind: "candidate" }> | undefined;
    let resultCategory: LifecycleErrorCategory | undefined;
    try {
      const preparation = await this.options.store.withConnectionIssuanceLock(
        ...
        async (store): Promise<RenewalPreparation> => {
          ...
          if (!current || current.localDisabledAt) {
            resultCategory = "invalid_active_credential";
          ...
          if (
            ...
            resultCategory = "insufficient_scope";
          ...
          if (
            ...
            resultCategory = "renewal_expired";
      ...
      if (issuedPreparation) {
        ...
        issuedPreparation = undefined;
        let current: ConnectionRecord | null;
        let generations: CredentialGenerationRecord[];
        try {
          [current, generations] = await Promise.all([
            this.options.store.getConnection(this.providerId, identity.issuer, identity.subject),
            this.options.store.listPrincipalCredentialGenerations(
              this.providerId,
              identity.issuer,
              identity.subject,
            ),
        ...
        if (current?.credentialGenerationId !== rejected.candidate.id) {
          if (!generations.some((generation) => generation.id === rejected.candidate.id)) {
            // The issuance transaction rolled back or its COMMIT outcome was
            // unknown. Re-establish custody after that transaction has ended.
            await this.acquireCredentialCustody(identity, rejected.candidate);
          }
      ...
      this.metric({
        ...
        operation,
        outcome: "failure",
        category: lifecycleCategory(error) ?? "persistence_failure",
        durationMs: performance.now() - lockStartedAt,
        value: 1,
      });
      await this.emit(
  ...
  private async persistReauthorizationRequired(
    ...
    if (!saved) throw generationConflict();
    if (current.phase !== "reauthorization_required") {
      this.metric({
        name: "lifecycle_phase_transition",
        provider: this.providerId,
        from: current.phase,
  ...
  private metric(metric: ConnectionLifecycleMetric): void {
    try {
      this.options.metrics?.record({ ...metric, diagnosticId: this.options.diagnosticId });
    } catch {
...
function lifecycleCategory(error: unknown): LifecycleErrorCategory | undefined {
  return error instanceof ProviderLifecycleError || error instanceof ProviderToolScopeError
    ? error.category
    : undefined;
}
