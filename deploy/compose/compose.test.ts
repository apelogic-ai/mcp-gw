...
describe("Docker Compose deployment skeleton", () => {
  test("defines gateway, google wrapper, and token store services", async () => {
    ...
    expect(compose).toContain(
      "image: ${AGENTGATEWAY_IMAGE:-ghcr.io/apelogic-ai/mcp-gw-agentgateway:0.4.10}",
    );
  ...
  test("provides an environment-neutral local Compose template", async () => {
    ...
    expect(envExample).toContain(
      "AGENTGATEWAY_IMAGE=ghcr.io/apelogic-ai/mcp-gw-agentgateway:0.4.10",
    );
    ...
    expect(envExample).toContain("GOOGLE_WORKSPACE_GOVERNANCE_CATALOG=");
    expect(envExample).toContain("GITHUB_MCP_GOVERNANCE_CATALOG=");
