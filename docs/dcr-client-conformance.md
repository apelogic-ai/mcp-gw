...
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Authorization-server discovery and protected-resource discovery | [RFC 8414](https://www.rfc-editor.org/rfc/rfc8414.html), [MCP authorization](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization) | Advertises exact issuer, endpoints, supported grants, S256, and exact MCP resource authority.                                                                                                                                                      | Authorization-route metadata tests and direct-client journey.                                                          |
| Dynamic public-client registration                              | [RFC 7591](https://www.rfc-editor.org/rfc/rfc7591.html)                                                                                                    | Accepts only `authorization_code` or the unordered set `authorization_code` + `refresh_token`; returns the canonical accepted metadata and no client secret.                                                                                       | DCR unit tests include the exact Codex 0.147.0/RMCP 3.0.0 shape and rejection cases.                                   |
| Native loopback redirect                                        | [RFC 8252](https://www.rfc-editor.org/rfc/rfc8252.html)                                                                                                    | Allows canonical HTTP loopback redirects with arbitrary ports only when the existing deployment opt-in is enabled; redirect matching remains exact after registration.                                                                             | DCR unit tests and direct-client journey.                                                                              |
| Authorization code and PKCE                                     | [RFC 6749](https://www.rfc-editor.org/rfc/rfc6749.html), [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636.html)                                           | Requires authorization code, exact redirect URI, state/nonce binding, and PKCE S256; codes are one-time and short-lived.                                                                                                                           | Broker and route tests plus direct-client journey.                                                                     |
| Resource-bound access                                           | [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html)                                                                                                    | Requires the exact canonical resource at authorization, code exchange, and refresh; emitted JWT audience remains exact.                                                                                                                            | Broker binding tests and renewed-token runtime-authentication journey.                                                 |
| Refresh grant and rotation                                      | [RFC 6749](https://www.rfc-editor.org/rfc/rfc6749.html), [OAuth security best current practice](https://www.rfc-editor.org/rfc/rfc9700.html)               | Issues a refresh credential only to a dynamic client registered for the grant. Stores only its SHA-256 digest, preserves client/principal/resource/scope binding, rotates on every use, prevents scope widening, and revokes the family on replay. | In-memory and SQL-store tests, real PostgreSQL concurrent-rotation smoke, route tests, and five-minute-expiry journey. |
| Public client authentication                                    | [RFC 7591](https://www.rfc-editor.org/rfc/rfc7591.html)                                                                                                    | Advertises `token_endpoint_auth_method=none`; rejects HTTP Authorization credentials and form client secrets/assertions.                                                                                                                           | Authorization-route tests.                                                                                             |
